...
export async function generateTa6(input: Ta6Input): Promise<Buffer> {
  const blocks: Block[] = [
    ...
    { kind: 'qa', q: '1.1  Looking towards the property from the road, who owns or accepts responsibility for the boundary on the left?', a: input.boundaryOwnership },
    { kind: 'qa', q: '1.2  On the right?', a: 'Shared / unknown.' },
    { kind: 'qa', q: '1.3  At the rear?', a: 'Shared with neighbour at 25 Wynyard Terrace.' },
    { kind: 'qa', q: '1.4  Have any boundary features been moved or altered in the last ten years?', a: 'No.' },
    { kind: 'qa', q: '1.5  Has there been any dispute about a boundary, fence, or hedge?', a: input.disputes },

    { kind: 'h2', text: '2. Disputes and complaints' },
    { kind: 'qa', q: '2.1  Have there been any disputes or complaints regarding this property or a property nearby?', a: input.disputes },
    { kind: 'qa', q: '2.2  Is the seller aware of anything which might lead to a dispute about the property or a property nearby?', a: 'No.' },

    { kind: 'page-break' },

    { kind: 'h2', text: '3. Notices and proposals' },
    { kind: 'qa', q: '3.1  Have any notices or correspondence been received from a neighbour, council, or government department which affect the property in any way (e.g. notices concerning planning, road improvements, party walls)?', a: 'No notices received.' },
    ...
    { kind: 'qa', q: '5.1  Are there any guarantees or warranties in respect of the property?', a: input.guarantees.length > 0 ? input.guarantees.join('; ') : 'None.' },
    { kind: 'qa', q: '5.2  Insurance claims in the last 3 years?', a: input.insurance },

    { kind: 'h2', text: '6. Environmental matters' },
    { kind: 'qa', q: '6.1  Has any part of the property ever been affected by flooding?', a: input.flooding },
    { kind: 'qa', q: '6.2  Is the seller aware of any contamination or radon affecting the property?', a: 'No.' },
    { kind: 'qa', q: '6.3  Is Japanese knotweed present at the property or nearby?', a: input.knotweed },

    { kind: 'page-break' },

    { kind: 'h2', text: '7. Rights and informal arrangements' },
    { kind: 'qa', q: '7.1  Are there any arrangements or rights of way over neighbouring land used to access the property?', a: input.rightsOfWay },
    { kind: 'qa', q: '7.2  Parking arrangements?', a: input.parking },

    { kind: 'h2', text: '8. Services' },
    { kind: 'qa', q: '8.1  Is the property connected to the following services: gas, electricity, water, telephone, broadband?', a: input.servicesNotes ?? 'Gas, electricity, water and broadband connected.' },
    { kind: 'qa', q: '8.2  Drainage — is the property connected to mains foul drainage?', a: input.drainageMains },
    { kind: 'qa', q: '8.3  Is the property connected to mains surface water drainage?', a: input.drainageMains },
    ...
    { kind: 'h2', text: '9. Occupiers' },
    {
      ...
      a: input.occupiersOver17.length > 0 ? input.occupiersOver17.join(', ') : 'None — the seller is the sole occupant.',
    },
    {
      kind: 'qa',
