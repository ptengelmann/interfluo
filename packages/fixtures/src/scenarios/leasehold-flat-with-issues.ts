...
export async function leaseholdFlatWithIssues(): Promise<ScenarioBundle> {
  ...
  const ta6 = await generateTa6({
    propertyAddress,
    sellerName,
    boundaryOwnership: 'The seller believes the boundary on the left is the responsibility of the neighbour at Flat 2, but is not certain.',
    disputes:
    ...
    guarantees: ['NHBC certificate has long expired. No FENSA or electrical guarantees are available.'],
  ...
  const ta7 = await generateTa7({
    propertyAddress,
    sellerName,
    landlordName: 'Wynyard Terrace Freehold Limited (registered office: 12 St James Square, London SW1Y 4LB).',
    ...
    buildingsInsurance: 'Buildings insurance is arranged by the Landlord; the premium is recharged through the service charge.',
  ...
  const ta10 = await generateTa10({
    ...
    items: [
      ...
      { item: 'Curtains and curtain rails — living room', status: 'Excluded', notes: 'The seller wishes to take these.' },
      { item: 'Carpets throughout', status: 'Included' },
      { item: 'Light fittings (excluding chandelier in hallway)', status: 'Included' },
      { item: 'Hallway chandelier', status: 'For sale (extra)', notes: '£600 if buyer wishes to acquire.' },
  ...
  const drainage = await generateDrainageSearch({
    ...
    publicSewerWithinBoundary: 'No public sewer is recorded within the boundary of the property. A private foul sewer runs beneath the rear courtyard.',
